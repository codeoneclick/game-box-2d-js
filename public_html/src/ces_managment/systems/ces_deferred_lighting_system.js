...
gb.ces_deferred_lighting_system.prototype.on_feed_end = function() {
    for (var i = 0; i < this.m_light_casters; ++i) {
        ...
        for (var j = 0; j < this.m_shadow_casters.length; ++j) {
            var convex_hull_component = this.m_shadow_casters[i].get_component(gb.ces_component_type.convex_hull);
            var shadow_caster_transformation_component = this.m_shadow_casters[i].get_component(gb.ces_component_type.transformation);

            var shadow_caster_matrix_m = new gb.mat4().identity();
            var shadow_caster_parent = this.m_shadow_casters[i].parent;

            while (shadow_caster_parent) {
                var shadow_caster_parent_transformation_component = shadow_caster_parent.get_component(gb.ces_component_type.convex_hull);
            ...
            shadow_caster_matrix_m = shadow_caster_matrix_m * shadow_caster_transformation_component.matrix_m;
            light_mask_component.update_mask_geometry(shadow_caster_matrix_m, convex_hull_component.oriented_vertices);
            light_component.add_shadow_caster(this.m_shadow_casters[i]);
        }
...
gb.ces_deferred_lighting_system.prototype.update_recursively = function(entity) {
    ...
    var shadow_component = entity.get_component(gb.ces_component_type.shadow);
    if (shadow_component) {
